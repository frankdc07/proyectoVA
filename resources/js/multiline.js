...
//Formatter functions for the axes
    chartObj.formatAsNumber = d3version3.format(".0f");
    chartObj.formatAsDecimal = d3version3.format(".2f");
    chartObj.formatAsCurrency = d3version3.format("$.2f");
    chartObj.formatAsFloat = function (d) {
        if (d % 1 !== 0) {
            return d3version3.format(".2f")(d);
        } else {
            return d3version3.format(".0f")(d);
        }
    ...
    chartObj.bisectYear = d3version3.bisector(chartObj.xFunct).left; //< Can be overridden in definition

//Create scale functions
    chartObj.xScale = d3version3.scale.linear().range([0, chartObj.width]).domain(d3version3.extent(chartObj.data, chartObj.xFunct)); //< Can be overridden in definition

// Get the max of every yFunct
    chartObj.max = function (fn) {
        return d3version3.max(chartObj.data, fn);
    };
    chartObj.yScale = d3version3.scale.linear().range([chartObj.height, 0]).domain([0, d3version3.max(chartObj.yFuncts.map(chartObj.max))]);

    chartObj.formatAsYear = d3version3.format("");

//Create axis
    chartObj.xAxis = d3version3.svg.axis().scale(chartObj.xScale).orient("bottom").tickFormat(chartObj.xFormatter); //< Can be overridden in definition

    chartObj.yAxis = d3version3.svg.axis().scale(chartObj.yScale).orient("left").tickFormat(chartObj.yFormatter); //< Can be overridden in definition
...
// Build line building functions
    ...
    for (var yObj in yObjs) {
        yObjs[yObj].line = d3version3.svg.line().interpolate("cardinal").x(function (d) {
...
// Change chart size according to window size
    chartObj.update_svg_size = function () {
        ...
        d3version3.selectAll(".focus.line").attr("y2", chartObj.height);
    ...
    chartObj.bind = function (selector) {
        chartObj.mainDiv = d3version3.select(selector);
        ...
        chartObj.chartDiv = d3version3.select(chartSelector);
        d3version3.select(window).on('resize.' + chartSelector, chartObj.update_svg_size);
...
// Render the chart
    chartObj.render = function () {
        ...
        return chartObj;
        function mousemove() {
            var x0 = chartObj.xScale.invert(d3version3.mouse(this)[0]), i = chartObj.bisectYear(dataset, x0, 1), d0 = chartObj.data[i - 1], d1 = chartObj.data[i];
            try {
            ...
            focus.select(".focus.year").text("Año: " + chartObj.xFormatter(chartObj.xFunct(d)));
        }
