...
function RadarChart(id, data, options) {
	...
	data.forEach(function(d){
		d[value + "Average"] = d3version3.mean(d.values, function(e){ return e[value] }); 
	})
	...
	var maxValue = Math.max(cfg.maxValue, d3version3.max(data, function(i){
		return d3version3.max(i.map(
	...
	function cellover(d) {
			...
			d3version3.selectAll(".radarArea")
				...
				.style("fill-opacity", 0.1); 
			//Bring back the hovered over blob
			d3version3.select("." + data[d][0][areaName].replace(/\s+/g, ''))
				.transition().duration(200)
				.style("fill-opacity", 0.7);	
	}
  ...
  //d3version3 symbol creates a path-string, for example
  //"M0,-8.059274488676564L9.306048591020996,
  //8.059274488676564 -9.306048591020996,8.059274488676564Z"
  	.shape("path", d3version3.svg.symbol().type("triangle-up").size(150)())
  	.shapePadding(10)
  	.scale(cfg.color)
